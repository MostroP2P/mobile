...
- For logging, always use the pre-configured singleton `logger` instance via `import 'package:mostro_mobile/services/logger_service.dart';`. Direct instantiation of `Logger()` is no longer permitted. Refer to `docs/architecture/LOGGING_SYSTEM.md` for detailed guidelines.
