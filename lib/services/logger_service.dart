...
void addLogFromIsolate(Map<String, dynamic> logData) {
  // Only capture to memory if logging is enabled
...
class MemoryLogOutput extends LogOutput {
  ...
  /// Controls whether logs are captured to memory buffer
  /// When false, logs still appear in console but are not stored
  static bool isLoggingEnabled = false;

  @override
  void output(OutputEvent event) {
    // Early return if logging is disabled - avoids all processing overhead
...
Logger get logger {
  _cachedLogger ??= Logger(
    ...
    output: _MultiOutput(
      ...
      Config.isDebug ? ConsoleOutput() : null, // Console only in debug mode
    ),
    level: Config.isDebug ? Level.debug : Level.warning, // Higher level in release
