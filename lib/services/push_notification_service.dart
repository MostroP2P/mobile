...
///
/// This implements a privacy-preserving approach inspired by MIP-05:
/// - Device tokens are encrypted with the server's public key
/// - Server only knows the mapping trade_pubkey -> device_token temporarily
...
class PushNotificationService {
  ...
  Future<bool> initialize() async {
    if (!isSupported) {
      _logger.i('Push notifications not supported on this platform');
      return false;
    }
    ...
    try {
      final response = await http
          .get(
            Uri.parse('$_pushServerUrl/api/info'),
          )
      ...
      if (response.statusCode == 200) {
        ...
        _logger.i(
            'Push server initialized, pubkey: ${_serverPubkey?.substring(0, 16)}...');
        return true;
      } else {
  ...
  Future<bool> registerToken(String tradePubkey) async {
    if (!isSupported || _serverPubkey == null) {
      _logger.w('Push service not initialized or not supported');
      return false;
    }
    ...
    try {
      ...
      _logger
          .d('Registering token for trade: ${tradePubkey.substring(0, 16)}...');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          _logger.i(
              'Token registered successfully for trade ${tradePubkey.substring(0, 16)}...');
          return true;
        }
      }
  ...
  Future<bool> unregisterToken(String tradePubkey) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        _logger.i(
            'Token unregistered for trade ${tradePubkey.substring(0, 16)}...');
        return true;
      }
  ...
  ///
  /// Format: ephemeral_pubkey (33 bytes) || nonce (12 bytes) || ciphertext (236 bytes)
  /// Total: 281 bytes
